...
# soft constraint check with penalty  mechanism
def soft_constraint_check(state, student):
    penalty = 0
    for stu in student:
        temp = [0,0,0]
        course = stu.course
...
if __name__ == '__main__':
    ...
    for r in room:
        for i in range(len(exam)):
            ...
            heapq.heappush(queue,[soft_constraint_check(temp_exam,stu),random.random(),temp_exam])
    flag = False
    cnt = 0
    # find solution via priority queen
    while len(queue) > 0:
        cur_exam = heapq.heappop(queue)[2]
        cnt += 1
